...
export const validateUpdateProfile = [
  ...
  body("timeZone")
    ...
    .custom((value) => {
      try {
        ...
        throw new Error("Time zone must be a valid IANA time zone (e.g. Asia/Kolkata)");
      }
    }),
  ...
  body("refreshToken")
    .not()
    .exists()
    .withMessage("Refresh token cannot be updated through this endpoint"),
  body("isEmailVerified")
    .not()
    .exists()
    .withMessage("Email verification status cannot be updated through this endpoint"),
...
export const validateUpdatePreferences = [
  body("language")
    ...
    .withMessage("Invalid language format (e.g. en, en-US)"),
...
export const validateConvertBatch = [
  ...
  body("amounts.*.amount").isFloat({ min: 0 }).withMessage("Each amount must be a positive number"),
